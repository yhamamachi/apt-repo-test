...
EXTRA_APT_REPO="\
deb [arch=arm64 trusted=yes signed-by=/etc/apt/trusted.gpg.d/kernel-repo.asc] https://raw.githubusercontent.com/yhamamachi/apt-repo-test/debian12 bookworm sparrow-hawk \
"
...
parted ./debian.img mklabel msdos mkpart primary ext4 1MiB 100%
SEEK=$(fdisk -l debian.img | grep img1 | awk '{print $2}')
SECTORS=$(fdisk -l debian.img | grep img1 | awk '{print $4}')
...
dd if=rootfs.ext4 of=debian.img bs=512 seek=${SEEK} conv=notrunc
gzip -f ./debian.img
